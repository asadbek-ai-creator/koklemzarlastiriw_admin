...
function SurvivalBadge({ task }: { task: WateringTask }) {
  ...
  return <Badge variant={variant}>{t('watering.survival', { rate: rate.toFixed(1) })}</Badge>;
}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {t('watering.completedCount', { completed: completedCount, total: sorted.length })}
            </span>
          </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-semibold">
                        {t('watering.day', { interval: task.day_interval })}
                      </span>
                      <span className="text-sm text-muted-foreground">
                        — {t('watering.scheduled', { date: new Date(task.scheduled_date).toLocaleDateString() })}
                      </span>
                      {task.is_completed && (
                        <Badge variant="outline">{t('watering.completed')}</Badge>
                      )}
                    ...
                    {task.is_completed ? (
                      <div className="grid grid-cols-2 gap-x-6 gap-y-1 rounded-md border bg-muted/30 p-3 text-sm sm:grid-cols-4">
                        <div>
                          <span className="text-muted-foreground">{t('watering.alive')}</span>
                        ...
                        <div>
                          <span className="text-muted-foreground">{t('watering.water')}</span>
                          <p className="font-medium">
                            {task.water_used_ltr?.toLocaleString()} L
                          </p>
                        </div>
                        <div>
                          <span className="text-muted-foreground">{t('watering.cost')}</span>
                          <p className="font-medium">
                        ...
                        {task.photos && task.photos.length > 0 && (
                          <div className="col-span-full mt-2">
                            <span className="text-muted-foreground">{t('common.photos')}</span>
                            <PhotoGallery photos={task.photos} className="mt-1" />
                          </div>
                        )}
                      </div>
                    ) : (
